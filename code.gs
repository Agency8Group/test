...
const ADMIN_KEY = ""; // 관리 기능 보호용 키(선택). 비워두면 검증하지 않음
...
function doGet(e) {
  try {
    ...
    if (action === "getProducts") {
      ...
      if (!isAdminAuthorized_(params)) {
        response = { status: "error", message: "권한이 없습니다." };
      } else {
      ...
      if (!isAdminAuthorized_(params)) {
        ...
        if (!data || !data.products || !Array.isArray(data.products)) {
          response = response.status === "error" ? response : { status: "error", message: "products 배열이 필요합니다." };
        } else {
          response = handleSaveProducts(data.products, params);
        }
      }
    } else if (action === "ping") {
...
function handleGetProducts(params) {
  const productsSheetName = getProductsSheetName_(params);
  const sheet = getOrCreateSheet_(productsSheetName);
...
function handleGetOrders(params) {
  ...
  const orders = values.map(function (r) {
    return {
      depositorName: r[0] != null ? String(r[0]) : "",
      contact: r[1] != null ? String(r[1]) : "",
      product: r[2] != null ? String(r[2]) : "",
      address: r[3] != null ? String(r[3]) : "",
      addressDetail: r[4] != null ? String(r[4]) : ""
    };
  });
...
function handleSaveProducts(products, params) {
  const productsSheetName = getProductsSheetName_(params);
  const sheet = getOrCreateSheet_(productsSheetName);
  ensureHeaders_(sheet, ["제품"]);
  ...
  if (clean.length > 0) {
    var rows = clean.map(function (n) { return [n]; });
    sheet.getRange(2, 1, rows.length, 1).setValues(rows);
  }

  SpreadsheetApp.flush();
  return { status: "success", message: "제품 목록이 저장되었습니다.", count: clean.length };
}
...
function isAdminAuthorized_(params) {
  if (!ADMIN_KEY || ADMIN_KEY.length === 0) {
    return true;
  }
  return params && params.adminKey && params.adminKey === ADMIN_KEY;
}
...
function getProductsSheetName_(params) {
  if (params && typeof params.productsSheet === 'string' && params.productsSheet.trim().length > 0) {
    return params.productsSheet.trim();
  }
  return SHEET_NAME_PRODUCTS;
}
